...
static const char *tun_dev_name = "dp_iot";
static const char *lock_file_path = "/tmp/dp_iot.lock";
...
static const char firewall_mangle_table_suffix[] = " PREROUTING -j DROP -m comment --comment \"dp_iot\" -p tcp --dport 80 -i ";
const size_t firewall_mangle_table_suffix_length = sizeof(firewall_mangle_table_suffix) - 1;

static const char const *firewall_add_forward_allow = "iptables -I FORWARD -m comment --comment \"dp_iot\" -i dp_iot -j ACCEPT";
static const char const *firewall_delete_forward_allow = "iptables -D FORWARD -m comment --comment \"dp_iot\" -i dp_iot -j ACCEPT 2>/dev/null";
...
int main(int argc, char *argv[]) {
    ...
    if (argc >= 3) {
        ...
        // Enable syslog
        if (argc == 4) {
            openlog("dp_iot", LOG_PID, LOG_DAEMON);
            logger = logToSyslog;
        }
        daemon(0, 0);
    }

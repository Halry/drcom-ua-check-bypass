...
static const char *tun_dev_name = "dp_pub";
static const char *lock_file_path = "/tmp/dp_pub.lock";
...
static const char firewall_mangle_table_suffix[] = " PREROUTING -j DROP -m comment --comment \"dp_pub\" -p tcp --dport 80 -i ";
const size_t firewall_mangle_table_suffix_length = sizeof(firewall_mangle_table_suffix) - 1;

static const char const *firewall_add_forward_allow = "iptables -I FORWARD -m comment --comment \"dp_pub\" -i dp_pub -j ACCEPT";
static const char const *firewall_delete_forward_allow = "iptables -D FORWARD -m comment --comment \"dp_pub\" -i dp_pub -j ACCEPT 2>/dev/null";
...
int main(int argc, char *argv[]) {
    ...
    if (argc >= 3) {
        ...
        // Enable syslog
        if (argc == 4) {
            openlog("dp_pub", LOG_PID, LOG_DAEMON);
            logger = logToSyslog;
        }
        daemon(0, 0);
    }

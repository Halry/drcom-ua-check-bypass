...
static const char *tun_dev_name = "dp_sre1";
static const char *lock_file_path = "/tmp/dp_sre1.lock";
...
static const char firewall_mangle_table_suffix[] = " PREROUTING -j DROP -m comment --comment \"dp_sre1\" -p tcp --dport 80 -i ";
const size_t firewall_mangle_table_suffix_length = sizeof(firewall_mangle_table_suffix) - 1;

static const char const *firewall_add_forward_allow = "iptables -I FORWARD -m comment --comment \"dp_sre1\" -i dp_sre1 -j ACCEPT";
static const char const *firewall_delete_forward_allow = "iptables -D FORWARD -m comment --comment \"dp_sre1\" -i dp_sre1 -j ACCEPT 2>/dev/null";
...
int main(int argc, char *argv[]) {
    ...
    if (argc >= 3) {
        ...
        // Enable syslog
        if (argc == 4) {
            openlog("dp_sre1", LOG_PID, LOG_DAEMON);
            logger = logToSyslog;
        }
        daemon(0, 0);
    }
...
static int packet_filter(char *data, size_t len) {
    ...
    static const char user_agent_replace_with[] = "IPTun";
...
#define buffer_size 4096
    ...
    if (user_agent_length >= buffer_size - 1) {
        ...
        logger("[%d] User-Agent: ", user_agent_length);
        logger("%s", string_buffer);
        logger(", ");
    }
    ...
    memcpy(user_agent_pointer + 2, user_agent_replace_with, sizeof(user_agent_replace_with) - 1);
    return 1;
}
